...
├── _worker.js                 # Main Pages Function entry point (Advanced Mode - ALL JS CODE INLINE)
...
### 1. `pages/_worker.js` (Single Combined File)
- **Purpose**: Main entry point for Pages Functions (Advanced Mode) - contains ALL JavaScript code
- **Location**: Must be named `_worker.js` in the output directory
- **Contents**: 
  - `wasm_exec.js` (inline) - WebAssembly execution runtime
  - `runtime.mjs` (inline) - Cloudflare Pages runtime context
  - Worker logic (inline) - Request routing and handling
- **Key Features**:
...
3. **Generate combined _worker.js**
   - Read `wasm_exec.js` content from tinywasm cache
   - Read and modify `runtime.mjs` template (configurable WASM filename)
   - Read worker template
   - Combine all content into single `_worker.js` file
   - Replace configurable placeholders (API prefix, WASM filename)
...
GoFlare embeds the template files using `go:embed`:
...
│   └── pages/
│       └── worker_combined.js    # Combined worker template
```

**Note**: `wasm_exec.js` files are read dynamically from tinywasm cache at runtime, not embedded.
...
└── pages/                     # Output directory (deploy this)
    ├── _worker.js             # Generated by goflare (contains ALL JS code)
    ├── app.wasm               # Generated by tinywasm
...
### Advanced Mode (single combined worker)
```
pages/_worker.js              ← Single file with ALL JS code inline
pages/app.wasm                ← Just the WASM binary
pages/index.html              ← Static assets
```

**Advanced Mode is now even simpler: only 2 files instead of 4!**
